...
EOF
    ...
    sub generate_taglist_varargs {
        ...
        # Object-like wrappers must fetch the base inside the helper to
        # preserve calls assembled from other macros. Under __NOLIBBASE__,
        # however, the global base has no declaration. Keep the base in the
        # function-like macro there, so merely including the header remains
        # valid and callers can provide a local base.
        my $emit_helper = sub {
            my ($explicit_base) = @_;
            ...
            if ($explicit_base) {
                print "void *const __base __asm(\"a6\"), ";
            }
            ...
            if ($pass_count == $fixed_count) {
                # All fixed parameters are real parameters of the base
                # function, so the array starts at the first variadic slot,
                # one past the last named one (e.g. EasyRequest's format
                # arguments).
            ...
            my $base = $explicit_base ? '__base' : $base_macro;

            if ($is_void) {
                print "    __${taglist_fname}_base($base, " .
                      join(", ", @call_args) . ");\n";
            } else {
                print "    return __${taglist_fname}_base($base, " .
                      join(", ", @call_args) . ");\n";
            }
            print "}\n\n";
        };

        print "#ifdef __NOLIBBASE__\n";
        $emit_helper->(1);
        print "#define $fname(...) " .
              "__${fname}_va($base_macro, __VA_ARGS__)\n";
        print "#else\n";
        $emit_helper->(0);
