...
EOF
    ...
    sub generate_taglist_varargs {
        ...
        # The wrapper is an object-like macro naming a real function, so
        # the base is fetched inside the helper. A function-like macro
        # breaks callers that build the call from macros expanded inside
        # another argument list, where the preprocessor cannot balance the
        # parentheses (MUI's WindowObject, ..., End and ReAction's
        ...
        if ($is_void) {
            print "    __${taglist_fname}_base($base_macro, " . join(", ", @call_args) . ");\n";
        } else {
            print "    return __${taglist_fname}_base($base_macro, " . join(", ", @call_args) . ");\n";
        }
        print "}\n\n";
